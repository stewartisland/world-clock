...
.DESCRIPTION
    The version lives in <Version> in WorldClock.csproj (the Windows app reads it at run time) and in
    MARKETING_VERSION in ios/WorldClock.xcodeproj/project.pbxproj (the iPhone app). Both are kept equal.
...
$pbxproj = Join-Path $root "ios/WorldClock.xcodeproj/project.pbxproj"
...
# The iPhone app uses the same version number.
$xcode = [IO.File]::ReadAllText($pbxproj)
if ($xcode -notmatch "MARKETING_VERSION = \d+\.\d+;") { throw "Couldn't find MARKETING_VERSION in $pbxproj" }
[IO.File]::WriteAllText($pbxproj, [regex]::Replace($xcode, "MARKETING_VERSION = \d+\.\d+;", "MARKETING_VERSION = $new;"))
...
if ($Commit) {
    git -C $root add WorldClock.csproj CHANGELOG.md ios/WorldClock.xcodeproj/project.pbxproj
